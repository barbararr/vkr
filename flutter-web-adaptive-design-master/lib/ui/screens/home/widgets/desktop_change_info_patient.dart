...
class UserChangingUser extends StatefulWidget {
  const UserChangingUser({Key? key}) : super(key: key);

  @override
  State<UserChangingUser> createState() => _UserChangingUserState();
}

class _UserChangingUserState extends State<UserChangingUser> {
  ...
  void _getData() async {
    userToChange = await ApiDataProvider().getUser(globals.user!.id);
    setState(() {
  ...
  List<String> categories = ['Нажмите для редактирования данных'];
    ...
    if (completed == false) {
      ...
      if (userData['roleId'].toString() == globals.doctorRoleId) {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => ((const DesktopPatientListPageDoctor()))));
      } else {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => ((const DesktopMainPatientPagePatient()))));
      }
    }
  }
